...
public static class GMLImporter
{
    ...
    private static void Import()
    {
        ...
        if (!string.IsNullOrEmpty(path))
        {
            ...
            foreach (var edge in graph.Edges)
            {
                ...
                float[] amounts = { 0.000f, 0.899f, 0.900f, 0.950f, 1.000f };
                float[] widths = { lineWidth, lineWidth, 1, 0, 0 };
                renderer.positionCount = amounts.Length;
                var curvePositions = new List<Vector3>(amounts.Length);
                var curveKeyframes = new List<Keyframe>(amounts.Length);
                for (var idx = 0; idx < amounts.Length; idx++)
                {
                    curvePositions.Add(Vector3.Lerp(goSource.transform.localPosition, goTarget.transform.localPosition,
                        amounts[idx]));
                    curveKeyframes.Add(new Keyframe(amounts[idx], widths[idx]));
                }

                renderer.SetPositions(curvePositions.ToArray());
                renderer.widthCurve = new AnimationCurve(curveKeyframes.ToArray());
...
public sealed class Graph
{
    ...
    public Graph(string gmlPath)
    {
        ...
        while (gmlContent[gmlIndex].Trim() == "node")
        {
            ...
            var label = string.Join(" ", tokensLabel.Skip(1)).Trim('"');
            gmlIndex++;
